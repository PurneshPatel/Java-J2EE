...
// pooled out DB cn rets to the pool n L1 cache is destroyed
	...
	@Override
	public String storeImage(String email, String filePath) throws IOException {
		String mesg = "storing of an image failed";
		String jpql = "select u from User u where u.email=:em";
		// get session from SF
		Session session = getFactory().getCurrentSession();
		// begin tx
		...
		try {
			...
			// => user :persistent
			// File validation
			// 1. create java.io.File class instance , that wraps file path
			File file = new File(filePath);
			if (file.exists() && file.isFile() && file.canRead()) {
				// user exists n file exists
				// File ---> byte[]
				user.setImage(FileUtils.readFileToByteArray(file));
				mesg = "storing of an image succeeded!";
			}
			tx.commit();
		} catch (RuntimeException e) {
			if (tx != null)
	...
	@Override
	public String restoreImage(int userId, String filePath) throws IOException {
		String mesg = "restoring of an image failed";
		// get session from SF
		Session session = getFactory().getCurrentSession();
		// begin tx
		Transaction tx = session.beginTransaction();
		try {
			...
			if(user != null)
			{
				...
				//create file class instance from the path
				File newFile=new File(filePath);
				//write byte[] image data ---> bin file
				FileUtils.writeByteArrayToFile(newFile, imageData);
				mesg="restoring of an image succeeded!";
			}
			tx.commit();
		} catch (RuntimeException e) {
			if (tx != null)
